...
class DisjunctionValuesSpec extends TestBase {
  ...
  "value on Disjunction" should {
    "should return the value inside a disjunction (\\/) if that disjunction is \\/- (right)" in {
      val r: String \/ String = \/.right(thisRecord)
      r.value should ===(thisRecord)
    }

    "should throw TestFailedException if that disjunction (\\/) is -\\/ (left) " in {
  ...
  "leftValue on Disjunction" should {
    ...
    "should throw TestFailedException if the disjunction is right" in {
